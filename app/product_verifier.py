...
def _call_openai(
    ...
    prompt = (
        ...
        "BELİRTİLMEMİŞ ≠ FARKLI (ÇOK ÖNEMLİ): Bir özellik (renk, beden, "
        "kapasite, 'Akıllı' gibi sıfatlar) kaynakta yazıp adayda hiç "
        "geçmiyorsa (aday sessiz kalmışsa), bu bir ÇAKIŞMA DEĞİLDİR -- "
        "satıcı o detayı başlığa yazmamış olabilir. Sadece aday AÇIKÇA "
        "FARKLI bir değer belirtiyorsa (ör. kaynak 'Beyaz' derken aday "
        "açıkça 'Siyah' diyorsa) reddet. Marka + model adı + ürün türü "
        "eşleşiyorsa ve aday sadece bazı sıfatları/detayları eksik "
        "bırakmışsa, bunu AYNI ürün kabul et.\n\n"
